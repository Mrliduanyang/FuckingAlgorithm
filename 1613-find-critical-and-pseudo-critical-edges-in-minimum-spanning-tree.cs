public class Solution {
    public IList<IList<int>> FindCriticalAndPseudoCriticalEdges(int n, int[][] edges) {
        ...
        for (var i = 0; i < m; ++i) //增加一个索引位
        {
            newEdges[i] = new int[4];
            for (var j = 0; j < 3; ++j) newEdges[i][j] = edges[i][j];
            newEdges[i][3] = i;
        }
        ...
        var value = 0;
        for (var i = 0; i < m; ++i) //求最小权值和
            if (ufStd.Unite(newEdges[i][0], newEdges[i][1]))
                value += newEdges[i][2];

        IList<IList<int>> ans = new List<IList<int>>();
        for (var i = 0; i < 2; ++i) ans.Add(new List<int>()); //答案增加两个新列表集合元素

        for (var i = 0; i < m; ++i) //遍历每条边
        {
            var uf = new UnionFind(n); //新的并查集
            var v = 0;
            for (var j = 0; j < m; ++j)

public class Solution {
    public double[] CalcEquation(IList<IList<string>> equations, double[] values, IList<IList<string>> queries) {
        ...
        for (var i = 0; i < values.Length; i++) {
            ...
            if (map[dNode[a]].ContainsKey(dNode[b]) == false) map[dNode[a]].Add(dNode[b], values[i]);
            if (map[dNode[b]].ContainsKey(dNode[a]) == false) map[dNode[b]].Add(dNode[a], 1 / values[i]);
        }
    ...
    private double SearchResult(Dictionary<int, Dictionary<int, double>> map, bool[] visited, int start, int target,
        double cur) {
        if (start == target) return cur;
        visited[start] = true;
        var dNext = map[start];
        if (dNext.ContainsKey(target)) return SearchResult(map, visited, target, target, cur * dNext[target]);
